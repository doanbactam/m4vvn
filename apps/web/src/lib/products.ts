...
export const getQueueLength = (queueLength: number) => {
  const queueDays = Math.ceil(
    (queueLength / config.submissions.postingRate) * 7
  );
  const queueMonths = differenceInMonths(
    addDays(new Date(), queueDays),
    new Date()
  );
...
const getFeatureType = (featureName?: string) => {
  return Object.keys(SYMBOLS).find((key) =>
    featureName?.startsWith(SYMBOLS[key as SymbolType])
  ) as SymbolType | undefined;
};
