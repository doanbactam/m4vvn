...
export default async function Sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...
    ...categories.map((c) =>
      createEntry(`/categories/${c.slug}/self-hosted`, c.updatedAt)
    ),

    // Alternatives
    createEntry('/alternatives', now),
    ...alternatives.map((a) =>
      createEntry(`/alternatives/${a.slug}`, a.updatedAt)
    ),

    // Topics
    ...config.site.alphabet
      .split('')
      .map((letter) => createEntry(`/topics/letter/${letter}`, now)),
