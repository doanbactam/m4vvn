...
export default async function AlternativePage(props: PageProps) {
  ...
  const categories = Object.values(
    tools.reduce<Record<string, { count: number; category: Category }>>(
      (acc, { categories }) => {
          ...
          <Section.Content className="gap-12 md:gap-14 lg:gap-16">
            <Prose>
              <p>
                ...
                {bestTools.shift()}. If that doesn't suit you, we've compiled a{' '}
                <Link href="/about#how-are-rankings-calculated">
                  ranked list
                </Link>{' '}
                of other open source {alternative.name} alternatives to help you
              ...
              {!!bestCategories.length && (
                <p>
                  ...
                  . Browse these if you want a narrower list of alternatives or
                  looking for a specific functionality of {alternative.name}.
                </p>
              )}
          ...
          <Section.Sidebar className="order-first md:order-last md:max-h-[calc(100vh-5rem)]">
            ...
            <InlineMenu
              items={tools.map(({ slug, name, faviconUrl }, index) => ({
                ...
                suffix: index < 3 && (
                  <AwardIcon className={medalColors[index]} />
                ),
              }))}
