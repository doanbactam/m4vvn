...
const badgeVariants = cva({
  ...
  variants: {
    variant: {
      primary:
        'bg-primary border-transparent text-background hover:[&[href]]:bg-primary/75',
      soft: 'bg-border/50 border-transparent hover:[&[href]]:bg-border/75 hover:[&[type]]:bg-border/75',
      outline:
        'bg-background hover:[&[href]]:bg-accent hover:[&[type]]:bg-accent',
      success:
        'bg-green-700/90 border-transparent text-background dark:bg-green-300/90',
      warning:
        'bg-yellow-700/90 border-transparent text-background dark:bg-yellow-300/90',
      danger:
        'bg-red-700/90 border-transparent text-background dark:bg-red-300/90',
    },
    size: {
    ...
    >
      <Slottable child={children} asChild={asChild}>
        {(child) => (
          <>
            {prefix && (
              <Slot.Root className={cx(badgeAffixVariants())}>
                {prefix}
              </Slot.Root>
            )}
            {child}
